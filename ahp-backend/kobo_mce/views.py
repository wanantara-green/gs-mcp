...
def _admin_ok(request) -> bool:
    """Validasi sandi admin (dikirim di body sbg 'admin_token')."""
    pwd = settings.ADMIN_PASSWORD
    return bool(pwd) and request.data.get("admin_token") == pwd


@api_view(["GET"])
def expert_detail(request, expert_id):
    """Detail satu ahli + seluruh nilai pairwise (untuk lihat/edit di admin)."""
    try:
        resp = ExpertResponse.objects.get(expert_id=expert_id)
    except ExpertResponse.DoesNotExist:
        return Response({"detail": "Ahli tidak ditemukan."},
                        status=status.HTTP_404_NOT_FOUND)
    pairwise = list(resp.pairwise_values.values("block", "i", "j", "value"))
    return Response({
        "expert_id": resp.expert_id, "nama": resp.nama, "instansi": resp.instansi,
        "tipologi": resp.tipologi, "is_valid": resp.is_valid,
        "submitted_at": resp.submitted_at, "catatan_validasi": resp.catatan_validasi,
        "pairwise": pairwise,
    })
...
def admin_verify(request):
    """Cek sandi admin (gerbang login panel admin frontend)."""
    if not _admin_ok(request):
        return Response({"detail": "Sandi admin salah atau admin nonaktif."},
...
def admin_save(request):
    """Tambah/edit (replace) satu ExpertResponse + pairwise. Wajib sandi admin."""
    if not _admin_ok(request):
        return Response({"detail": "Tidak diizinkan."},
                        status=status.HTTP_401_UNAUTHORIZED)
    ser = SubmitResponseSerializer(data=request.data)
    ser.is_valid(raise_exception=True)
    resp = _save_response(ser.validated_data)
...
def admin_delete(request):
    """Hapus satu ExpertResponse (cascade ke pairwise). Wajib sandi admin."""
    if not _admin_ok(request):
        return Response({"detail": "Tidak diizinkan."},
                        status=status.HTTP_401_UNAUTHORIZED)
    eid = request.data.get("expert_id", "")
    n, _ = ExpertResponse.objects.filter(expert_id=eid).delete()
    return Response({"deleted": n > 0, "expert_id": eid})
